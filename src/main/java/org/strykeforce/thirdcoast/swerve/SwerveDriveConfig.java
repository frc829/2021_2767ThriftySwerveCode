...
public class SwerveDriveConfig {
  ...
  public MotorControllerConfig driveConfig =
      new MotorControllerConfig(DriveMotorController.TALON_FX, FeedbackSensor.INTEGRATED_SENSOR);
  ...
  public Wheel[] getWheels() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      switch (azimuthConfig.azimuthController) {
        case VICTOR_SPX:
          azimuth = new VictorSPXWrapper(azimuthConfig, i+14);
          break;
        case TALON_SRX:
          azimuth = new TalonSRXWrapper(azimuthConfig, i+14);
          break;
        case SPARK_MAX:
          azimuth = new SparkMaxWrapper(azimuthConfig, i+14);
          break;
        default:
          azimuth = new TalonSRXWrapper(azimuthConfig, i+14);
          break;
